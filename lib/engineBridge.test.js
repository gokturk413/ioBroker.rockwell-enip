...
describe('engineBridge', () => {
	...
	describe('platformPackage', () => {
		it('names the optional dependency after the node platform-arch pair', () => {
			expect(platformPackage('win32', 'x64')).to.equal('iobroker.rockwell_ethernetip-win32-x64');
			expect(platformPackage('darwin', 'arm64')).to.equal('iobroker.rockwell_ethernetip-darwin-arm64');
			expect(platformPackage('darwin', 'x64')).to.equal('iobroker.rockwell_ethernetip-darwin-x64');
		});
	});
