...
const PACKAGE_PREFIX = 'iobroker.rockwell-enip';
 ...
 * @returns the npm package name, e.g. `iobroker.rockwell-enip-linux-arm64`
 */
...
function decompress(gzPath, log) {
	try {
		...
		try {
			...
			const dir = path.join(os.tmpdir(), 'iobroker.rockwell-enip-engine');
